...
export default async function UnitedStatesStatesPage({ params }: PageProps) {
  ...
  const { data: statePhotos } = unitedStates
    ? await supabase
        .from('travel_photos')
        .select('id, country_id, state_id, trip_id, image_url, image_hash, caption, location_name, latitude, longitude, taken_on, sort_order, is_featured, is_favorite_featured, created_at')
