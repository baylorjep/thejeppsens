...
export default async function CountryTravelPage({ params }: PageProps) {
  ...
  const [{ data: trips }, { data: photos }, { data: favorites }] = await Promise.all([
    supabase
      .from('travel_trips')
      .select('id, country_id, title, location_name, started_on, ended_on, notes, baylor_went, isabel_went')
      .eq('country_id', country.id)
      .order('started_on', { ascending: false, nullsFirst: false }),
    supabase
      .from('travel_photos')
      .select('id, country_id, trip_id, image_url, caption, location_name, taken_on, sort_order')
      .eq('country_id', country.id)
      .order('sort_order')
      .order('taken_on', { ascending: false, nullsFirst: false }),
    supabase
      .from('travel_favorites')
      .select('id, country_id, trip_id, type, name, location_name, latitude, longitude, notes, sort_order')
      .eq('country_id', country.id)
      .order('sort_order')
      .order('name'),
  ]);
